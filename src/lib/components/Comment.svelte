<script lang="ts">
	...
	let {
		comment = $bindable(), user, videoOwner, childResponseEditing = $bindable(false)
	}: {
		comment: any,
		user: {
			id: string,
	...
	let responsesCollapsed = $state(true);
	let newResponse = $state('');
	let isResponseEditing = $state(false);
	...
	function onreset() {
		const textarea = document.getElementById('new_response-' + comment.id) as HTMLTextAreaElement;
		const scrollPosition = window.scrollY;
		if (!textarea) return;
		textarea.style.height = 'auto';
		window.scrollTo(0, scrollPosition);
	...
	function handleResponse() {
		if (comment.parentCommentId) {
			...
			childResponseEditing = true;
		} else {
			responsesCollapsed = false;
			isResponseEditing = true;
		}
	}
</script>
...
<div class="flex gap-4 w-full">
	...
	<div class="flex flex-col w-full">
		...
		<div class="flex gap-2">
			...
			<button class="btn btn-sm btn-ghost" onclick={handleResponse}>Répondre
			</button>
		</div>
			...
			{:else}
				{#if comment.responses.length > 0}
					...
					{#each comment.responses as response}
						<Comment comment={response} {user} {videoOwner} bind:childResponseEditing />
					{/each}
				{/if}
			{/if}
		{/if}
		{#if (isResponseEditing || childResponseEditing) && !comment.parentCommentId}
			<form class="mt-2" action="?/comment" method="POST" use:enhance onreset={onreset}>
				<div class="flex flex-col">
					...
					<div transition:fade={{duration: 100}} class="flex justify-end gap-2 mt-2">
						<button class="btn btn-ghost btn-sm" type="reset" onclick={() => {
							isResponseEditing = false;
							childResponseEditing = false;
						}}>Annuler
						</button>
						<button class="btn btn-primary btn-sm" type="submit" disabled={newResponse.length < 1}
										onclick={() => {
											isResponseEditing = false;
											childResponseEditing = false;
										}}>Commenter
